...
function resetMasterFilter() {
    const picker = document.getElementById('master-month-picker');
...
function renderEarnings() {
    ...
    // --- Master Filter State ---
    const masterPicker = document.getElementById('master-month-picker');
    const masterClearBtn = document.getElementById('master-clear-filter');
    let filterStr = 'all';

    if (masterPicker && masterClearBtn) {
        if (masterPicker.value) {
            masterClearBtn.style.display = 'block';
            const [year, month] = masterPicker.value.split('-');
            const dateObj = new Date(year, month - 1);
            filterStr = dateObj.toLocaleString('default', { month: 'long', year: 'numeric' });

            // Apply Master Filter to Data
    ...
    // 1. Calculate Stats from Filtered Data
    completedOrders.forEach(order => {
        ...
        // Total Revenue (already filtered)
        totalRevenue += cost;
        ...
        // Product Duration
        if (!productDurationTotals[prodName]) productDurationTotals[prodName] = 0;
        productDurationTotals[prodName] += Math.abs(e - s);
    });
    ...
    if (monthlyContainer) {
        ...
        if (Object.keys(monthlyTotals).length === 0) {
            ...
            if (filterStr !== 'all') {
                monthlyContainer.innerHTML = '<div style="color:var(--text-secondary); font-style:italic;">Filtered by ' + filterStr + '</div>';
            }
            // Sort months? If filtered, only 1. If all, multiple.
            // If all, we might want to sort them.
    ...
    if (productContainer) {
        ...
        if (Object.keys(productTotals).length === 0) {
            ...
            if (filterStr !== 'all') {
                productContainer.innerHTML = '<div style="color:var(--text-secondary); font-style:italic;">Filtered by ' + filterStr + '</div>';
            }
    ...
    if (productDaysContainer) {
        ...
        if (Object.keys(productDurationTotals).length === 0) {
            ...
            if (filterStr !== 'all') {
                productDaysContainer.innerHTML = '<div style="color:var(--text-secondary); font-style:italic;">Filtered by ' + filterStr + '</div>';
            }
...
window.generatePDFReport = () => {
    ...
    const reportPicker = document.getElementById('master-month-picker');
    ...
    // Get Currency Code for PDF (Safe for Standard Fonts)
    const currencyCode = localStorage.getItem('zillow_currency') || 'USD';
    ...
    const monthlyTableData = sortedMonths.map(month => [month, `${currencyCode} ${monthlyTotals[month]}`]);
    ...
    sortedMonths.forEach(month => {
        ...
        Object.entries(productsInMonth).forEach(([prodName, amount]) => {
            productTableData.push([month, prodName, `${currencyCode} ${amount}`]);
        });
    });
